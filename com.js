...
const { TACTIC_INFO, needOf, fogged, bestScore, bestPossible, unseenPool } = require('./game');

const cmp = (a, b) => (a[0] - b[0]) || (a[1] - b[1]);

// フラッグ評価: カードを配置した場合の有利さをスコア化する
function evalPlacement(g, seat, fi, card) {
  const opp = 1 - seat;
  const f = g.flags[fi];
  const need = needOf(f);
  const fog = fogged(f);
  ...
  if (decisive && (!oppEval || cmp(my, oppEval) >= 0)) score += 500; // その場で獲得確定
  score += f.cards[seat].length * 3; // 既に投資しているフラッグを優先
  return score;
}

function evalEnv(g, seat, fi, key) {
  const f = g.flags[fi];
  const opp = 1 - seat;
...
function bestEnemyCardIdx(cards) {
  let bi = 0;
...
function cardKeepValue(card) {
  ...
  const order = { alexander: 9, darius: 9, cavalry: 8, shield: 5, traitor: 7, deserter: 6, redeploy: 5, scout: 4, fog: 3, mud: 2 };
  return order[card.key] || 3;
}

function pickDraw(g, seat) {
...
function chooseScoutReturn(g, seat) {
  const hand = g.hands[seat];
  const idxs = hand
  ...
  hand.forEach((card, handIdx) => {
    ...
    if (info.kind === 'morale') {
      ...
      g.flags.forEach((f, fi) => {
        ...
        candidates.push({ move: { type: 'tactic', handIdx, flag: fi, draw: pickDraw(g, seat) }, score: evalEnv(g, seat, fi, card.key) });
      });
      ...
      g.flags.forEach((f, fi) => {
        ...
        candidates.push({ move: { type: 'tactic', handIdx, flag: fi, idx, draw: pickDraw(g, seat) }, score: 10 + target.value });
      });
    } else if (card.key === 'traitor') {
      g.flags.forEach((f, fi) => {
        if (f.winner != null) return;
        f.cards[opp].forEach((c, idx) => {
          if (c.type !== 'troop') return;
          g.flags.forEach((dest, di) => {
            if (dest.winner != null || dest.cards[seat].length >= needOf(dest)) return;
            candidates.push({ move: { type: 'tactic', handIdx, flag: fi, idx, destFlag: di, draw: pickDraw(g, seat) }, score: 12 + c.value + dest.cards[seat].length });
          });
        });
      });
        ...
        f.cards[seat].forEach((c, idx) => {
          g.flags.forEach((dest, di) => {
            ...
            candidates.push({
              ...
              score: 4 + dest.cards[seat].length - f.cards[seat].length * 0.5 + c.value * 0.2,
            });
          });
