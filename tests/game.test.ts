...
describe('バトルライン: パス', () => {
  test('パスしてもカードを引かない(手札枚数が変わらない)', () => {
    const g = newGame();
    // 部隊カードを持たない手札ならパスが常に合法
    g.hands = [[{ id: 'x-fog', type: 'tactic', key: 'fog' } as Card], [troop('t1', 2, 9)]];
    g.troopDeck = [troop('deck1', 0, 1), troop('deck2', 0, 2)];
    g.turn = 0;
    const beforeCount = g.hands[0].length;
    const beforeDeck = g.troopDeck.length;

    const r = applyMove(g, 0, { type: 'pass', draw: 'troop' }, names);

    assert.equal(r.error, undefined, r.error);
    assert.equal(g.hands[0].length, beforeCount, 'パスした側の手札枚数は変化しないはず');
    assert.equal(g.troopDeck.length, beforeDeck, 'パスでは山札からカードを引かないはず');
  });
});

describe('バトルライン: 最終配置カードの記録(lastPlacement)', () => {
  test('部隊カードを配置すると lastPlacement が更新される', () => {
    const g = newGame();
    g.hands = [[troop('red1', 0, 5)], []];
    g.turn = 0;
    applyMove(g, 0, { type: 'troop', handIdx: 0, flag: 2 }, names);
    assert.deepEqual(g.lastPlacement && { seat: g.lastPlacement.seat, flag: g.lastPlacement.flag, cardId: g.lastPlacement.cardId }, {
      seat: 0,
      flag: 2,
      cardId: 'red1',
    });
  ...
  test('パスや脱走など配置を伴わない操作では lastPlacement は変化しない', () => {
    const g = newGame();
    g.hands = [
      [troop('red1', 0, 5)],
      [{ id: 'x-fog', type: 'tactic', key: 'fog' } as Card],
    ];
    g.turn = 0;
    applyMove(g, 0, { type: 'troop', handIdx: 0, flag: 2 }, names);
    const after = g.lastPlacement;
    g.turn = 1;
    applyMove(g, 1, { type: 'pass' }, names);
    assert.deepEqual(g.lastPlacement, after, 'パスでは lastPlacement は更新されないはず');
  });

  test('sanitize() は視点に応じて lastPlacement.who を you/opp に変換する', () => {
    const g = newGame();
    g.hands = [[troop('red1', 0, 5)], []];
    g.turn = 0;
    applyMove(g, 0, { type: 'troop', handIdx: 0, flag: 2 }, names);
    const asPlacer = sanitize(g, 0);
    const asOpponent = sanitize(g, 1);
    assert.equal(asPlacer.lastPlacement?.who, 'you');
    assert.equal(asOpponent.lastPlacement?.who, 'opp');
  });
});
