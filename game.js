...
const TACTIC_KEYS = Object.keys(TACTIC_INFO);
const COLOR_NAMES = ['赤', '橙', '黄', '緑', '青', '紫'];
const RANK_NAMES = ['ホスト(合計)', 'スカーミッシュ(連番)', '大隊(同色)', 'ファランクス(同数)', 'ウェッジ(同色連番)'];
...
function newGame() {
  const troopDeck = shuffle(allTroops());
  const tacticsDeck = shuffle(TACTIC_KEYS.map(k => ({ id: `x-${k}`, type: 'tactic', key: k })));
  const hands = [[], []];
  for (let i = 0; i < 7; i++) { hands[0].push(troopDeck.pop()); hands[1].push(troopDeck.pop()); }
  const flags = Array.from({ length: 9 }, () => ({
    cards: [[], []], env: [], winner: null, completedAt: [null, null],
  }));
  return {
    troopDeck, tacticsDeck, hands, flags,
    turn: Math.floor(Math.random() * 2),
    winner: null, winReason: null,
    tacticsPlayed: [0, 0], leaderUsed: [false, false],
    discard: [], pendingScout: null, seq: 1, log: [],
  };
}

/* ---------- 編成の評価 ---------- */

const needOf = f => (f.env.includes('mud') ? 4 : 3);
const fogged = f => f.env.includes('fog');
const cmp = (a, b) => (a[0] - b[0]) || (a[1] - b[1]);

function wildOptions(card) {
  if (card.type === 'troop') return [{ color: card.color, value: card.value }];
  const out = [];
  if (card.key === 'cavalry') { for (let c = 0; c < 6; c++) out.push({ color: c, value: 8 }); return out; }
  if (card.key === 'shield') { for (let c = 0; c < 6; c++) for (const v of [1, 2, 3]) out.push({ color: c, value: v }); return out; }
  for (let c = 0; c < 6; c++) for (let v = 1; v <= 10; v++) out.push({ color: c, value: v }); // leaders
  return out;
}

function assignments(cards) {
  let res = [[]];
...
function scoreConcrete(cards, fog) {
...
function bestScore(cards, need, fog) {
  let best = null;
...
function bestPossible(placedCards, need, fog, pool) {
  let best = null;
...
function bestCompleteConcrete(placed, need, fog, pool) {
  ...
  const topSum = k => poolValsDesc.slice(0, k).reduce((a, b) => a + b, 0);
  if (fog) return [0, psum + topSum(miss)];

  let best = [0, psum + topSum(miss)]; // ホストは常に可能
  const upd = s => { if (cmp(s, best) > 0) best = s; };
  ...
  if (distinct) {
    for (let lo = 1; lo <= 11 - need; lo++) {
      const win = []; for (let v = lo; v < lo + need; v++) win.push(v);
...
function syncCompletion(g, flag) {
...
function unseenPool(g, claimant) {
  const visible = new Set();
  for (const f of g.flags) for (const s of [0, 1]) for (const c of f.cards[s]) visible.add(c.id);
...
function resolveFlags(g, names) {
  for (let i = 0; i < g.flags.length; i++) {
    ...
    if (n0 >= need && n1 >= need) {
      ...
      const c = cmp(s0, s1);
      f.winner = c > 0 ? 0 : c < 0 ? 1 : (f.completedAt[0] < f.completedAt[1] ? 0 : 1);
...
function hasLegalTroopPlacement(g, seat) {
  return g.flags.some(f => f.winner == null && f.cards[seat].length < needOf(f));
}

function applyMove(g, seat, move, names) {
  ...
  if (move.type === 'pass') {
    ...
    endTurn(g, seat, move.draw, names, false);
    return {};
  }

  const card = hand[move.handIdx];
  if (!card) return { error: 'カードが見つかりません' };

  if (move.type === 'troop') {
    if (card.type !== 'troop') return { error: '部隊カードを選んでください' };
    const f = g.flags[move.flag];
    ...
    syncCompletion(g, f);
    g.log.push(`${names[seat]}がフラッグ${move.flag + 1}に${COLOR_NAMES[card.color]}${card.value}を配置`);
    endTurn(g, seat, move.draw, names, false);
    return {};
  }
  ...
  if (move.type === 'tactic') {
    ...
    if (info.kind === 'morale') {
      ...
      const target = f.cards[opp][move.idx];
      if (!target) return { error: '対象カードが見つかりません' };
      hand.splice(move.handIdx, 1);
      g.discard.push(card);
      f.cards[opp].splice(move.idx, 1);
      g.discard.push(target);
      syncCompletion(g, f);
      g.log.push(`${names[seat]}が【脱走】で相手のカードを除外`);
    } else if (k === 'traitor') {
      const f = g.flags[move.flag];
      const dest = g.flags[move.destFlag];
      ...
      const f = g.flags[move.flag];
      if (!f || f.winner != null) return { error: '未確定のフラッグを選んでください' };
      const target = f.cards[seat][move.idx];
      if (!target) return { error: '対象カードが見つかりません' };
      hand.splice(move.handIdx, 1);
      g.discard.push(card);
      f.cards[seat].splice(move.idx, 1);
      if (move.destFlag === -1) {
        g.discard.push(target);
      } else {
        const dest = g.flags[move.destFlag];
        if (!dest || dest.winner != null || dest.cards[seat].length >= needOf(dest)) {
          // ロールバック
          f.cards[seat].splice(move.idx, 0, target);
    ...
    if (info.kind !== 'guile' || k !== 'scout') {
      if (info.kind === 'morale' || info.kind === 'env') {
        g.log.push(`${names[seat]}が【${info.name}】を${'flag' in move ? `フラッグ${move.flag + 1}に` : ''}使用`);
      }
      g.tacticsPlayed[seat]++;
      endTurn(g, seat, move.draw, names, false);
    }
...
function endTurn(g, seat, drawPref, names, skipDraw) {
  ...
  if (!skipDraw) {
    ...
    if (deck.length) g.hands[seat].push(deck.pop());
  }
